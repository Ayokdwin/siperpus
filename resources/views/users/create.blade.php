    ...
    <main class="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50 dark:bg-slate-950">
        <div class="w-full max-w-6xl mx-auto space-y-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-semibold text-slate-900 dark:text-slate-100">Tambah DataPengguna</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">Buat akun baru untuk admin, petugas, atau anggota</p>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <form action="{{ route('users.store') }}" method="POST" class="p-6 space-y-5">
                    ...
                    {{-- Role --}}
                    <div x-data="{ role: '{{ old('role', 'anggota') }}' }">
                        ...
                        <div x-show="role === 'anggota'" x-cloak class="mt-5">
                            ...
                            <input type="text" id="kode_anggota" name="kode_anggota" value="{{ old('kode_anggota') }}"
                                @readonly(true)
                    ...
                    <div>
                        ...
                        <textarea id="alamat" name="alamat" rows="3"
                            ...
                            class="w-full rounded-lg border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-700
                                   ...
                                   focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none
                                   dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:focus:ring-emerald-500/20
                                   transition-colors @error('alamat') border-rose-300 @enderror">{{ old('alamat') }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
