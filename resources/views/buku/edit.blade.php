    ...
    <main class="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50 dark:bg-slate-950">
        <div class="max-w-5xl mx-auto space-y-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-semibold text-slate-900 dark:text-slate-100">Edit Buku</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">Perbarui informasi buku yang ada</p>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <form action="{{ route('buku.update', $buku->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select id="kategori_id" name="kategori_id" required
                                ...
                                <option value="" disabled {{ old('kategori_id', $buku->kategori_id) ? '' : 'selected' }}>
                                    Pilih kategori
                                </option>
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4"
                            ...
                            class="w-full rounded-lg border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-700
                                   ...
                                   focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none
                                   dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:focus:ring-emerald-500/20
                                   transition-colors @error('deskripsi') border-rose-300 @enderror">{{ old('deskripsi', $buku->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
