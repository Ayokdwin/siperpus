    ...
    <main class="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50 dark:bg-slate-950">
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-xl font-semibold text-slate-900 dark:text-slate-100">Dashboard Petugas</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">Ringkasan transaksi yang kamu tangani</p>
                </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex items-center justify-between">
                        <p class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-slate-500">Transaksi Hari Ini</p>
                    ...
                    <p class="mt-2 text-2xl font-semibold text-slate-800 dark:text-slate-100">{{ $transaksiHariIni }}</p>
                    <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">yang kamu proses</p>
                </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <p class="mt-2 text-2xl font-semibold text-slate-800 dark:text-slate-100">{{ $aktifDiproses }}</p>
                    <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">belum dikembalikan</p>
                </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <p class="mt-2 text-2xl font-semibold text-rose-600 dark:text-rose-400">{{ $terlambatDiproses }}</p>
                    <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">perlu ditagih dendanya</p>
                </div>

                <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex items-center justify-between">
                        <p class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-slate-500">Keranjang Aktif</p>
                    ...
                    <p class="mt-2 text-2xl font-semibold text-slate-800 dark:text-slate-100">{{ $jumlahKeranjang }}</p>
                    @if ($jumlahKeranjang > 0)
                        <a href="{{ route('peminjam.checkout') }}" class="mt-1 inline-block text-xs font-medium text-emerald-600 hover:underline dark:text-emerald-400">
                            Lanjutkan checkout →
                        </a>
                    @else
                        <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">belum ada item</p>
                    @endif
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                {{-- Transaksi terbaru yang diproses petugas ini --}}
                <div class="lg:col-span-2 rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
                        <h2 class="text-sm font-semibold text-slate-700 dark:text-slate-200">Transaksi Terbaru</h2>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-4 py-10 text-center text-slate-400 dark:text-slate-500">
                                            ...
                                            Kamu belum memproses transaksi apa pun.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
